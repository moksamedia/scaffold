...
- `tests/outline-store.test.js` - Pinia store integration: CRUD, outline ops, undo/redo, persistence, legacy migration, plus `findOrphanedMediaForProjectRemoval` orphan-set semantics, the `deleteProject(id, { purgeRemoteMedia })` shared-bucket purge path, and the cache→remote backfill helpers (`mediaBackendSupportsRemoteSync`, `getUnsyncedMediaForProject`, `getAllUnsyncedMedia`, `backfillMediaToRemote`)
...
- `tests/media-cached-adapter.test.js` - Read-through cache semantics: cache hits, lazy promotion, write-through to remote, plus `localGcOnly` mode, the `forceDeleteFromRemote` escape hatch (with and without the underlying `remote.forceDelete`), and `backfillRemoteFromCache` (cache→remote push, idempotency, whitelist mode, partial-failure accounting, progress callbacks)
