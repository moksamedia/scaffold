...
- `tests/json-export-import.test.js` - JSON export/import validation, normalization, roundtrip, media payload round-trip, and long-note collapsed-color settings + per-note `collapsedBgColor` round-trip
...
- Long-note collapsed-background colors (per-note tint with project-scoped palette):
  - Each long note can carry a `collapsedBgColor` hex; the value only renders when the note is collapsed (the wrapper falls back to the default `#f5f5f5` when expanded), so the editor's working surface stays neutral while the closed previews can be color-coded for skim-reading.
  - Project settings track a `longNoteColorRoot` hex (defaults to `DEFAULT_LONG_NOTE_COLOR_ROOT = '#80aaff'`) and a `longNoteRecentCustomColors` array (max 5, deduped). `generateComplementaryPalette(rootColor)` derives a deterministic 6-swatch palette by rotating around the color wheel in 60° steps with saturation clamped into a soft pastel band — every swatch reads as a background tint regardless of how saturated the root is.
  - The long-note editor dialog in `OutlineItem.vue` exposes a "Collapsed background color" row with: the 6 generated swatches, a "Palette root" `<input type="color">` that regenerates the palette, a "Custom" `<input type="color">` that selects a one-off color and pushes it onto the recent-customs list, the recent-customs row of swatches, and a "Clear" button. Selecting a swatch on an already-saved note flushes through `setLongNoteBackground` immediately so the closed preview updates without waiting for autosave; new (unsaved) notes carry the selection in dialog state until the first save/autosave creates the note.
  - `outline-store.js` migration in `loadFromStorage` defaults the new fields on legacy projects and normalizes any malformed values via `normalizeHexColor`. `createProject` seeds them on every new project, including the example project.
  - JSON / `.scaffoldz` round-trip: `exportAsJSON` always writes `longNoteColorRoot` (defaulted) and `longNoteRecentCustomColors` (filtered through `normalizeHexColor`), and writes `collapsedBgColor` on long notes only when set. `importFromJSON` fills color defaults on legacy payloads without those fields.
