...
- Notes management (short/long notes) with visibility controls. `addLongNote(itemId, text, { collapsedBgColor })` accepts an optional collapsed-background hex (validated and normalized via `long-note-palette.js`); `setLongNoteBackground(itemId, noteId, color)` toggles or clears the per-note color (passing `null`/`undefined` clears, an invalid string is ignored to protect against in-flight UI inputs). `setLongNoteColorRoot(value)`, `setLongNoteBgOpacity(value)`, and `pushLongNoteRecentCustomColor(value)` keep the project's palette settings (`longNoteColorRoot`, `longNoteBgOpacity`, `longNoteRecentCustomColors`) in sync with the dialog.
...
### `/src/utils/color/`
- `long-note-palette.js` - Pure color helpers powering the long-note collapsed background picker. `normalizeHexColor(value)` (lowercases, prefixes `#`, expands `#abc` → `#aabbcc`, returns `null` for invalid input), `isValidHexColor`, `generateComplementaryPalette(rootColor)` (deterministic 6-swatch palette derived by rotating around the color wheel in 60° steps with saturation clamped to a soft pastel band and lightness pinned to ~0.88 so all swatches read as backgrounds), and `pushRecentCustomColor(list, color)` (immutable, dedupes case-insensitively, caps at `MAX_RECENT_CUSTOM_COLORS = 5`). `DEFAULT_LONG_NOTE_COLOR_ROOT = '#80aaff'` is the fallback root for legacy projects.

### `/src/utils/logging/`
- `logger.js` - Centralized structured logger with three levels (`debug`, `info`, `error`). API: `logger.debug(event, payload)`, `logger.info(event, payload)`, `logger.error(event, errorOrPayload, extra?)`. Output schema: `{ level, event, ts, ...payload }`. Sensitive keys (`secret`, `password`, `passphrase`, `authorization`, `bearer`, `*accessKey*`, `signature`, `cookie`) are redacted recursively from payloads. Errors are normalized to `{ errorName, errorMessage, errorStack? }` (stack only when level is `debug` or `setIncludeStackInProd(true)`). Long string fields are truncated to ~2000 chars; cycles and deep nesting are guarded. Level gating defaults to `info` (or `debug` when `import.meta.env.DEV` / `VITE_LOG_LEVEL` / `LOG_LEVEL` indicates dev mode), and can be overridden at runtime via `localStorage.setItem('scaffold-log-level', 'debug')` (the `LOG_LEVEL_OVERRIDE_KEY` constant). An in-memory ring buffer (default 200 entries) is exposed via `getRecentLogs()` / `clearLogs()` and powers the **Settings → Diagnostics** copy/download workflow for user issue reports. Tests can swap the underlying sink with `setSink(fn)` and assert against `getRecentLogs()`; the global test setup silences the default sink and resets the level/buffer between tests.
...
- `tests/json-export-import.test.js` - JSON export/import validation, normalization, roundtrip, media payload round-trip, and long-note collapsed-color settings + per-note `collapsedBgColor` + project `longNoteBgOpacity` round-trip / legacy hoisting
- `tests/long-note-palette.test.js` - Pure palette helpers: hex normalization (including `#abc` shorthand), `generateComplementaryPalette` determinism + 6-color shape + light-band invariant + invalid-input fallback to `DEFAULT_LONG_NOTE_COLOR_ROOT`, and `pushRecentCustomColor` dedupe / cap / invalid-input behavior
...
- `tests/logger.test.js` - Logger contract: level gating, redaction (case-insensitive, recursive), error normalization (including `AbortError`), string truncation, circular-reference safety, ring buffer rotation, sink override, `LOG_LEVEL_OVERRIDE_KEY` export
...
- Long-note collapsed-background colors (per-note tint with project-scoped palette):
  - Each long note can carry a `collapsedBgColor` hex; **tint strength** is project-wide (`settings.longNoteBgOpacity`, default full strength). The color layers over the default `#f5f5f5` surface per that opacity — it applies everywhere the tint shows (collapsed wrapper + expanded header strip).
  - Project settings track a `longNoteColorRoot` hex (defaults to `DEFAULT_LONG_NOTE_COLOR_ROOT = '#80aaff'`), `longNoteBgOpacity` (clamped 0…1, omitted from storage when default), and a `longNoteRecentCustomColors` array (max 5, deduped). `generateComplementaryPalette(rootColor)` derives a deterministic 6-swatch palette by rotating around the color wheel in 60° steps with saturation clamped into a soft pastel band — every swatch reads as a background tint regardless of how saturated the root is.
  - The long-note editor dialog in `OutlineItem.vue` exposes a "Background color" row with: the 6 generated swatches, a "Palette root" `<input type="color">` that regenerates the palette, a "Custom" `<input type="color">` that selects a one-off color and pushes it onto the recent-customs list, a **Strength (project)** slider that updates `longNoteBgOpacity` for the whole project, the recent-customs row of swatches, and a "Clear" button. Selecting a swatch on an already-saved note flushes through `setLongNoteBackground` immediately so the closed preview updates without waiting for autosave; new (unsaved) notes carry the selection in dialog state until the first save/autosave creates the note.
  - `outline-store.js` migration in `loadFromStorage` defaults the new fields on legacy projects, hoists legacy per-note `collapsedBgOpacity` into `longNoteBgOpacity` once, strips note-level opacity, and normalizes malformed values via `normalizeHexColor` / `clampOpacity`. `createProject` seeds them on every new project, including the example project.
  - JSON / `.scaffoldz` round-trip: `exportAsJSON` always writes `longNoteColorRoot` (defaulted), `longNoteRecentCustomColors` (filtered through `normalizeHexColor`), and clamped `longNoteBgOpacity`; writes `collapsedBgColor` on long notes only when set. Legacy per-note `collapsedBgOpacity` in imported JSON is hoisted into `longNoteBgOpacity` when the project setting is absent. `importFromJSON` fills color defaults on legacy payloads without those fields.
...
- Structured application logging (`src/utils/logging/logger.js`):
  - Three levels: `debug` (noisy internals), `info` (lifecycle milestones / operation summaries), `error` (failures and unexpected exceptions). All logs share the schema `{ level, event, ts, ...payload }` with stable dot-style event ids (e.g. `app.init.success`, `context.switch.failed`, `media.backend.select.failed`, `media.gc.success`, `import.success`, `export.docx.failed`, `project.delete.success`, `persist.save.failed`, `version.restore.failed`, `media.upload.image.failed`).
  - All previously ad-hoc `console.warn` / `console.error` callsites in `outline-store.js`, `media/index.js`, `media/cached-adapter.js`, `context/migration.js`, `context/session.js`, `export/docx.js`, `SettingsDialog.vue`, `ContextSwitcher.vue`, `ProjectsSidebar.vue`, `OutlineEditor.vue`, and `OutlineItem.vue` are routed through the logger so they share the same schema, redaction, and ring buffer.
  - Lifecycle observability is added at the high-value async boundaries: `initPromise`, `initContexts`, `hydrateActiveContext`, `loadFromStorage`, `switchContext` (with `fromContextId` / `toContextId` / `durationMs` / `projectsLoaded`), `createNewContext`, `deleteProject`, `triggerMediaGc` (with `gcReason` / `deleted` / `kept` / `skippedByGrace`), `reselectMediaBackend`, `selectMediaAdapter` (per-tier success/fallback events), and the import/export entrypoints (with `format`, `fileSizeBytes`, `projectsImported`, `versionsImported`, `mediaImported`, `warningsCount`).
  - Sensitive keys (`secret`, `password`, `passphrase`, `authorization`, `bearer`, `*accessKey*`, `signature`, `cookie`) are redacted recursively from payloads before they hit the sink. Errors are normalized to `{ errorName, errorMessage, errorStack? }` (stack only when `debug` or `setIncludeStackInProd(true)`); long strings are truncated; cycles are guarded.
  - Level gating defaults to `info` (or `debug` when `import.meta.env.DEV` / `VITE_LOG_LEVEL` / `LOG_LEVEL` indicate dev mode). Support staff can ask end users to enable verbose logs at runtime via `localStorage.setItem('scaffold-log-level', 'debug')` (constant `LOG_LEVEL_OVERRIDE_KEY`).
  - In-memory ring buffer (default 200 entries) backs the **Settings → Program Settings → Diagnostics** section in `SettingsDialog.vue`. The user can **Copy diagnostics** (clipboard) or **Download diagnostics** (`scaffold-diagnostics-<ISO>.json`); the payload bundles `capturedAt`, `activeContextId`, `mediaBackend`, `storeReady`, `projectCount`, `userAgent`, and the ring buffer entries. **Clear log buffer** is exposed for users who want to reset before reproducing an issue.
  - Tests: `tests/logger.test.js` covers level gating, redaction (case-insensitive, recursive into arrays + objects), error normalization (including `AbortError` → `isAbort: true`), string truncation, circular-reference safety, ring buffer rotation, and the `LOG_LEVEL_OVERRIDE_KEY` export. `tests/setup.js` silences the default sink and resets the level/buffer between every test so app-side logs never bleed into other suites' output.
